...
\section{Langage et syntaxe}
  ...
  \begin{discussion}[Substitution]
    Étant données deux expressions $\metalng{\alpha}$ et $\metalng{\xi}$ et une variable $\metalng{x}$, on définit $\metalng{\xi}\subst{\metalng{x}}{\metalng{\alpha}}$ comme étant l'expression obtenue en remplaçant dans $\metalng{\xi}$ toutes les feuilles étiquetées par $\metalng{x}$ par l'arbre $\metalng{\alpha}$.
  \end{discussion}
